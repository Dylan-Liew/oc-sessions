...
import { selectManyWithSearch, selectWithSearch } from "../output/prompt.js";
...
export async function selectSessionIds(
  sessions: RootSession[],
  options: SelectSessionOptions = {},
): Promise<string[]> {
  const { intro } = options;
  ...
  return selectedIndexes.map((index) => sessions[index].sessionId);
}
